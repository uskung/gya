...
\section{Härledning av Euler-Lagrange ekvationen} \label{appendix:euler_lagrange_equation}
%\addcontentsline{toc}{section}{A - Härledning av Euler-Lagrange ekvationen}
    ...
    \begin{equation}
        S = \int_{t_1}^{t_2}\left(T(t)-V(t)\right)dt = \int_{t_1}^{t_2}\Lagr dt.
    \end{equation}
    Eftersom $\Lagr$ är beroende av positionen $x$, hastigheten $\dot{x}$ och tiden $t$, kan vi uttrycka lagrangianen $\Lagr$ som funktion av dessa variabler:
    \begin{equation}
        S = \int_{t_1}^{t_2} \Lagr(x,\dot{x},t) dt.\label{appendix:action_definition}
    \end{equation}
    ...
    \begin{align}
        ...
        &= \int_{t_1}^{t_2} \left( \frac{\partial \Lagr}{\partial x_a} \frac{\partial x_a}{\partial a} + \frac{\partial \Lagr}{\partial \dot{x}_a} \frac{\partial \dot{x}_a}{\partial a}\right) dt. \label{appendix:after_chainrule}
    \end{align}
    ...
    \begin{align}
         \frac{\partial}{\partial a} S[x_a(t)] &= \int_{t_1}^{t_2} \left(\frac{\partial \Lagr}{\partial x_a} \beta + \frac{\partial \Lagr}{\partial \dot{x}_a} \dot{\beta}\right) dt \notag \\
         &= \int_{t_1}^{t_2} \frac{\partial \Lagr}{\partial x_a}\beta\: dt + \int_{t_1}^{t_2} \frac{\partial \Lagr}{\partial \dot{x}_a} \dot{\beta}\: dt. \label{appendix:split_integral}
    \end{align}
    Genom att partiellt integrera den högra termen i (\ref{appendix:split_integral}) får vi:
    \begin{align}
        \frac{\partial}{\partial a} S[x_a(t)] &= \int_{t_1}^{t_2} \frac{\partial \Lagr}{\partial x_a}\beta\: dt + \frac{\partial \Lagr}{\partial \dot{x}_a} \beta - \int_{t_1}^{t_2} \left( \frac{d}{dt} \frac{\partial \Lagr}{\partial \dot{x}_a} \right) dt \notag \\
        &= \int_{t_1}^{t_2} \left( \frac{\partial \Lagr}{\partial x_a} - \frac{d}{dt} \frac{\partial \Lagr}{\partial \dot{x}_a} \right)\beta\: dt + \frac{\partial \Lagr}{\partial \dot{x}_a} \beta\:\biggr|_{t_1}^{t_2} \label{appendix:after_partial_integration}
    \end{align}
...
\section{Kod för simulering av dubbelpendeln} \label{appendix:code}
