...
Eulers stegmetod är den enklaste explicita metoden för att bestämma lösningar till differentialekvationer med begynnelsevillkor. Metoden grundar sig i att dela in en differentialekvationen i flera diskreta tidsintervall $h$. Eftersom Eulers stegmetod är en metod av första ordningen, innebär det att felet vid varje tidsteg är proportionell mot kvadraten av tidssteget $h$. 

\subsubsection{Definition av Eulers stegmetod}

Givet en differentialekvation på formen $y' = f(t,y)$ med det kända begynnelsevillkoret $y(t_0)$o och det diskreta tidsteget $h$, definieras nästa värde $y_{n+1}$ enligt:
\begin{align}
...
\subsection{Behandling av rörelseekvationerna för simulering}
...
För att däremot kunna använda (\ref{eq:equations_of_motion_numerical_section}) med Runge-Kuttametoden, måste de skrivas om till ordinära differentialekvationer på formen $y' = f(t,y)$. Därför genomfördes  substitutionen $\dot{\theta}_1 = \omega_1$ och $\dot{\theta}_2 = \omega_2$, vilket gav:
\begin{equation}
    \begin{cases}
        \dot{\theta}_1 &= \omega_1  \\
        \dot{\theta}_2 &= \omega_2 \\
