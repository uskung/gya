...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Däremot är fortfarande tredje och fjärde ekvationerna i (\ref{eq:equations_of_motion_with_omega_numerical_section}) sammankopplade\footnote{I engelsk litteratur benämns denna typ av ekvationer som 'coupled'.}. För att kunna använda (\ref{eq:equations_of_motion_with_omega_numerical_section}) med numeriska simuleringar gjordes substitutionen enligt (\ref{eq:greek_substitutions}). 
\begin{equation}
    \begin{cases}
...
För att kunna simulera dubbelpendeln definierades en tillståndsvektor \\$\mathbf{X} = [\theta_1, \omega_1, \theta_2, \omega_2]$, vilket därmed ger att $\dot{\mathbf{X}} = [\omega_1, \dot{\omega}_1, \omega_2, \dot{\omega}_2]$. Ett program skrevs sedan i Python med hjälp av dessa tillståndsvektorer, först med Eulers-stegmetod, sedan med Runge-Kuttametoden, för att simulera en dubbelpendel. Dessa jämfördes enligt begynnelsevinklarna i tabell \ref{tab:euler_rk4_comparision} att demonstrera effektiviteten av respektive metod. 
...
Sedan simulerades en pendel med startvillkoren $\theta_1 = 2.9$ och $\theta_2 = -3.1$ [rad] under 180 s. Vinklarna $\theta_1$ och $\theta_2$ plottades parametriskt mot varandra, samt plottades de som funktion av tiden. Därefter simulerades en annan pendel på exakt liknande sätt, men med begynnelsevinklarna $\theta_1 = 0.7$ och $\theta = 0.2$ [rad].
