...
Resultatet av simuleringarna som beskrevs i sektion \ref{section:numeriska_metoder} redovisades i figurerna nedan. Första simuleringen, där Eulers-stegmetod och Runge-Kuttametoden jämfördes, redovisades i figur \ref{fig:comparision_euler_rk4_small_angle} och \ref{fig:comparision_euler_rk4_large_angle}. Simuleringen där två pendlar simulerades med RK4-metoden, där den andra pendeln hade 0.00001\% större begynnelsevinklar, redovisades i figur \ref{fig:comparision_between_small_angle_perbutation}. Resultatet från de parametriska plottarna av $\theta_1$ och $\theta_2$, samt plottarna av vinklarna $\theta_1$ och $\theta_2$ som funktion av tiden enligt tabell \ref{tab:angles_for_parametric}, redovisades i figur \ref{fig:parametric_plots} respektive figur \ref{fig:plots_as_function_of_time}. Simuleringen av pendlarna som simulerades i 180 s sammanställdes i figur \ref{fig:plots_of_chaotic_pendulum_over_long_time} och \ref{fig:plots_of_stable_pendulum_over_long_time}. Till sist sammanställdes resultatet från simuleringen med begynnelsevinklarna $\theta_1 = 2.899$, $\theta_2 = 1.913$ och $\theta_1 = 2.226$, $\theta_2 = -2.114$ [rad] i figur \ref{fig:plots_of_stable_pendulums}. 
...
\begin{figure}[ht]
    ...
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
        ...
        \end{subfigure}
    \captionsetup{width=0.80\linewidth}
    \caption{Parametrisk plot av vinklarna $\theta_1$ och $\theta_2$ som funktion av tiden för dubbelpendlar som simulerades 30 s med RK4-metoden med steglängden $h = 0.00005$ s. Startvinklarna för $\theta_1$ och $\theta_2$ anges i respektive bildtext.}
...
\begin{figure}
    ...
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
        ...
        \end{subfigure}
        \captionsetup{width=0.80\linewidth}
        \caption{Plot av vinklarna $\theta_1$ och $\theta_2$ som funktion av tiden. Alla dubbelpendlar simulerades i 20s, förutom i figur \ref{subfig:plots_as_function_of_time_B}, där den simulerades i 30 s. Simuleringen genomfördes med RK4-metoden med steglängden $h = 0.00005$ s.}
...
\begin{figure}[h!]
    \begin{adjustwidth}{-3cm}{-3cm}
        ...
        \end{subfigure}
        \captionsetup{width=0.9\linewidth}
        \caption{Plot av dubbelpendel med startvillkoren $\theta_1 = 0.7$ och $\theta_2 = 0.2$ [rad]. Simuleringen genomfördes med RK4-metoden i 180 s, men för figur \ref{subfig:plots_of_stable_pendulum_over_long_time_A} plottades 90 s för att göra kurvan mer läsbar. }
...
\begin{figure}[ht]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
        ...
        \end{subfigure}
        \captionsetup{width=0.80\linewidth}
        \caption{Simulering av pendlar med begynnelsevillkoren $\theta_1 = 2.899$ och $\theta_2 = 1.913$ [rad] (figur \ref{subfig:plots_of_stable_pendulums_A}, \ref{subfig:plots_of_stable_pendulums_C} och \ref{subfig:plots_of_stable_pendulums_E}), samt $\theta_1 = 2.226$ och $\theta_2 = -2.114$ [rad] (figur \ref{subfig:plots_of_stable_pendulums_B}, \ref{subfig:plots_of_stable_pendulums_D} och \ref{subfig:plots_of_stable_pendulums_F}). Figur \ref{subfig:plots_of_stable_pendulums_A} och \ref{subfig:plots_of_stable_pendulums_B} visar den streckade linjen den färdväg som pendeln tagit. Figur \ref{subfig:plots_of_stable_pendulums_C} och \ref{subfig:plots_of_stable_pendulums_D} visas en parametrisk plot för pendlarna. Figur \ref{subfig:plots_of_stable_pendulums_E} och \ref{subfig:plots_of_stable_pendulums_F} visar vinklarna $\theta_1$ och $\theta_2$ som funktion av tiden. Alla simuleringar genomfördes med RK4-metoden under 30 s, där steglängden $h = 0.00005$ s.}
