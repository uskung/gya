...
\begin{align}
    ...
    (\ref{eq:x_1:dot}) \leftrightarrow (\ref{eq:y_2:dot}) \Rightarrow &= \frac{1}{2}m_1\left(\dot{\theta}_1^2 l_1^2 \cos^2{\theta_1} + \dot{\theta}_1^2 l_1^2 \sin^2{\theta_1}\right) + \frac{1}{2}m_2
    \left(
...
Differentialekvationerna i (\ref{eq:equations_of_motion}) är icke-linjära och av andra ordningen, vilket gör de omöjligt att lösa rent analystiskt. Därmed måste ekvationerna lösas numeriskt. Detta arbete berör två typer av numeriska lösningsmetoder för differentialekvationer: \emph{Eulers stegmetod} och \emph{Runge-Kuttametoden} av fjärde ordningen. 
...
Runge-Kuttametoden av fjärde ordningen\footnote{Egentligen är Runge-Kuttametoderna en familj av metoder, då den innefattar ett flertal olika metoder, bland annat Eulers stegmetod. Däremot brukas RK4-metoden menas med benämningen ''Runge-Kuttametoden'', vilket är den som användes här. För vidare förklaring, se \cite{Runge-Kutta_method/Butcher2008}. } är annan vanlig metod för att lösa differentialekvationer, dock till bättre precision än exempelvis Eulers stegmetod. Metoden bygger på att approximera lösningen stegvis med ett litet tidssteg $h$, men istället för att bara beräkna nuvarande lutningen (det som sker i Eulers stegmetod), så beräknas flera mellanliggande lutningar, där ett viktad medelvärde istället dras \cite{runge_kutta_methods_2026}. Konsekvensen blir att felmarginalen blir betydligt mindre hos RK4-metoden än hos Eulers stegmetod. Till skillnad från Eulers stegmetod, där felet minskar proportionellt mot steglängden $h$, så minskar felet mot steglängden upphöjt till 4 \cite{runge_kutta_methods_2026}. 
